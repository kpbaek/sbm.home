...
CREATE TABLE IF NOT EXISTS `message` (
  `no` int(11) NOT NULL AUTO_INCREMENT,
  `send_id` varchar(50) NOT NULL COMMENT '발송자아이디',
  `resv_id` varchar(50) NOT NULL COMMENT '받는아이디',
  `reg_date` datetime NOT NULL COMMENT '등록일',
  `read_date` datetime NOT NULL COMMENT '본날',
  `contents` text NOT NULL COMMENT '내용',
  `ip` varchar(15) NOT NULL COMMENT '작성자 ip 주소',
  `is_delete_r` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '발송자 삭제 여부',
  `is_delete_s` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '수신자 삭제 여부',
  PRIMARY KEY (`no`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='쪽지'